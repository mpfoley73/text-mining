...
At this point the words can reveal some of their relative importance through their TF-IDF statistic. A term's frequency (TF) is its proportion of the terms in the document. The inverse document frequency (IDF) is the log of the inverse ratio of documents in which the term appears. The product of TF and IDF (TF-IDF) indicates how important a term is to a document within the corpus. For this project the chapters of TMS are the documents within the TMS corpus. For each chapter, each word's TF-IDF increases with its frequency in the chapter and decreases with the number of chapters in which it appears.
...
tms_tokens %>%
  ...
  filter(id == 1) %>%
  ggplot(aes(x = tf, y = idf, size = tf_idf, label = word, color = tf_idf)) +
  ?geom_point(alpha = 0.2, show.legend = FALSE, color = few_pal()(8)[3]) +
  geom_text(check_overlap = TRUE) +
  theme_minimal() +
  labs(title = "TF-IDF of the first chapter of TMS, Of Sympathy",
       subtitle = "Larger words are most 'important'.")
```
...
tms_tf_idf %>%
  mutate(word = reorder_within(word, by = tf_idf, within = Chapter)) %>%
  ggplot(aes(x = word, y = tf_idf, fill = as.factor(Chapter))) +
  geom_col(alpha = 0.8, show.legend = FALSE) +
  scale_fill_few() +
  facet_wrap(~ Chapter, scales = "free_y", ncol = 3) +
  scale_x_reordered() +
  ...
  labs(x = NULL, y = NULL,
       ...
       subtitle = "Suggesting thematic elements covered in each chapter")
```
