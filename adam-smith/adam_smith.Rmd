...
Next I pivoted the data into a *sparse matrix* with one row per chapter (55 rows), one column per word (1,090 cols), with cell values equal to the word's frequency count. The matrix is `r scales::percent(sum(tms_sparse %>% as.matrix() == 0) / (55*1090), accuracy = 1)` zeros. Now the data is ready for modeling.
